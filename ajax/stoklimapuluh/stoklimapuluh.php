<?php
	...
	if(empty($jumlah['total'])){
		...
		$no		= 1;
		$tabel	= '<h6>Produk Stok <50</h6><table class="table table-bordered"><thead><tr><th><center>No.</center></th><th>Produk</th><th>Sediaan</th><th><div align="right">Total Stok</div></th><th><div align="right">Minimal Stok</div></th><th><div align="right">Gudang</div></th><th>Satuan Qty.</th></tr></thead>';
		$master	= $conn->prepare("SELECT A.jumlah, A.gudang, B.nama_pro, B.berat_pro, B.minstok_pro, C.nama_kpr, C.satuan_kpr, D.nama_spr FROM(SELECT id_pro, SUM(sisa_psd) AS jumlah FROM produk_stokdetail GROUP BY id_pro) AS A LEFT JOIN produk AS B ON A.id_pro=B.id_pro LEFT JOIN kategori_produk AS C ON B.id_kpr=C.id_kpr LEFT JOIN satuan_produk AS D ON B.id_spr=D.id_spr WHERE A.jumlah<=B.minstok_pro");
		$master->bindParam(':limit', $limit, PDO::PARAM_INT);
		$master->execute();
		while($hasil	= $master->fetch(PDO::FETCH_ASSOC)){
				$tabel	.= '<tr><td><center>'.$no.'</td></center><td>'.$hasil['nama_pro'].'</td><td>'.$hasil['nama_kpr'].' ('.$hasil['berat_pro'].' '.$hasil['nama_spr'].')</td><td><div align="right">'.$data->angka($hasil['jumlah']).'</div></td><td><div align="right">'.$data->angka($hasil['minstok_pro']).'</div></td><td>'.$hasil['gudang'].'</td><td>'.$hasil['satuan_kpr'].'</td></tr>';
		$no++;
		}
		$tabel	.= '</table>';
	}
