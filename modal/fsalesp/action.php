		...
		switch($menu){
			case 'faktur':
				...
				try {
                    ...
                    if (is_array($dRead)) {
						array_push($msgBugs, "Nomor Penerimaan  ".$nomorpo." atau Nomor Penerimaan sudah pernah digunakan!");
					}
				...
				if (empty($msgBugs)) {
					...
					try {
						...
						if (is_array($hcek)) {
							array_push($msgBugs, "ID Penerimaan tidak dapat diproses!");
						}
					} catch (PDOException $e) {
						array_push($msgBugs, $e->getMessage());
					}
			...
			case 'delete':
				...
				try {
					$master	= $conn->prepare("SELECT id_psd, jumlah_tfd FROM transaksi_fakturdetail_p WHERE id_tfk=:kode");
				...
				*/
				...
				if (empty($msgBugs)) {
					try {
						$riwayat= $conn->query("INSERT INTO riwayat (kode_riwayat, menu_riwayat, status_riwayat, ket_riwayat, created_at, created_by) VALUES('$kode', 'Faktur Pinjaman', 'Delete', '', '$catat', '$admin')");
					} catch (PDOException $e) {
						array_push($msgBugs, $e->getMessage());
					}
				}
