	...
	if($secu->validadmin($admin, $kunci)==false){
		...
		switch($menu){
			...
			case "update":
				...
				$tanggal_tkf	= $secu->injection($_POST['tanggal_tkf']);
				$status			= $secu->injection($_POST['status']);
				$edit			= $conn->prepare("UPDATE tuker_faktur SET id_kot=:id_kot, id_tfk=:id_tfk, tanggal_tkf=:tanggal_tkf, status=:status, updated_at=:catat, updated_by=:admin WHERE id_tkf_t=:kode");
